...
### Steps required:
- In the `dependencies` section in your project's `package.json`, upgrade `gatsby` to `^3.0.0` and `gatsby-source-sanity` to `^7.0.0`
- Add `"gatsby-plugin-image": "^1.0.0"` as a dependency in your `package.json`.
...
The helper methods `getFluidGatsbyImage` and `getFixedGatsbyImage` has been removed in favor of `getGatsbyImageData()` which is based on [`gatsby-plugin-image`](https://www.gatsbyjs.com/plugins/gatsby-plugin-image) and supports a number of cool new features and performance optimizations.
